...
#if defined(NDEBUG)
# error "Birucoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xbb66a75fc344eac4362ddc998283d103984d4bda44ad984e7d660e65e60233eb");
...
const string strMessageMagic = "Birucoin Signed Message:\n";
...
{
    // Birucoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Birucoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Birucoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x44a271b0ee0031165820fa613a515c791482f0a0cd431d8c061b786f81f5987e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("049e7c0934036a785a4cd6f5418fb7bb68e2a386fa61b5a4849eea763483499d33f111089a3b260384a0a26b5ab57622c791879725a8012e6280a1eba38c3c75ba") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1539107180;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 246836;

        if (fTestNet)
        {
            block.nTime    = 1539107200;
            block.nNonce   = 1902702;
        }
...
unsigned char pchMessageStart[4] = { 0xc3, 0xd1, 0xb3, 0xbd }; // Birucoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BirucoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BirucoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BirucoinMiner(CWallet *pwallet)
{
    printf("BirucoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("birucoin-miner");
    ...
    try { loop {
        ...
        printf("Running BirucoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BirucoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BirucoinMiner, pwallet));
}
